...
function CustomersPage() {
  ...
  useEffect(() => {
    async function loadCustomers() {
      setLoading(true);
      try {
        const response = await fetch(`${API_URL}/orders/vendor/customers`, {
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-border">
                {loading && (
                  <tr><td colSpan={7} className="px-4 py-6 text-center text-sm text-muted-foreground">Loading customers...</td></tr>
                )}
                {!loading && customers.length === 0 && (
                  <tr><td colSpan={7} className="px-4 py-6 text-center text-sm text-muted-foreground">No customers yet.</td></tr>
                )}
                {customers.map((c) => {
