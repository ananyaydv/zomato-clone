...
function formatVendorOrder(order) {
  const customerLabel = order.user ? `Customer ${String(order.user.mobile).slice(-4)}` : 'Guest';
  return {
    id: order.orderNumber,
    customer: customerLabel,
    phone: order.user ? order.user.mobile : '',
    items: order.items.map((item) => ({
      name: item.name,
      qty: item.quantity,
      price: item.price,
...
async function GetVendorOrders(req, res) {
  try {
    const orders = await Order.findAll({
      ...
      include: [{ model: OrderItem, as: 'items' }, { model: Vendor, as: 'vendor' }, { model: User, as: 'user' }],
      order: [['createdAt', 'DESC']],
    });
    res.json(orders.map(formatVendorOrder));
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
}
...
async function GetVendorCustomers(req, res) {
  try {
    ...
    orders.forEach((order) => {
      ...
      const city = (order.address || '').split(',').pop().trim() || 'Unknown';
      const existing = customerMap.get(key);

      if (!existing) {
        customerMap.set(key, {
          id: `CUST${order.user.id}`,
          name: `Customer ${String(order.user.mobile).slice(-4)}`,
    ...
    const customers = Array.from(customerMap.values()).map((c) => ({
      ...c,
      lastOrder: new Date(c.lastOrder).toISOString().split('T')[0],
    }));
...
module.exports = { CreateOrder, ConfirmPayment, GetMyOrders, GetVendorOrders, GetVendorCustomers };
